...
#
# Usage:
#   ./push-images.sh                    # build multi-platform + push all
#   ./push-images.sh --push-only        # docker push existing local images (single-arch only)
#   ./push-images.sh --build-only       # build multi-platform, no push
#   ./push-images.sh --service kiosk-ui # build + push one service only
...
if [ "$BUILD" = false ]; then
  ...
  section "Building & Pushing (${PLATFORMS})"
  FAILED=(); PUSHED=()
  ...
  for def in "${SERVICE_DEFS[@]}"; do
    ...
    if [ "$PUSH" = true ]; then
      ...
      BUILD_CMD+=(--load)   # load into local docker (single-arch only)
      warn "  --build-only: loading amd64 only into local docker"
    fi
...
section "Done 🚀  Multi-platform manifests pushed to Docker Hub"
